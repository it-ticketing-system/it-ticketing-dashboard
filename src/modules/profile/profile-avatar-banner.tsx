...
interface ProfileAvatarBannerProps {
  user: IUser;
  isPending: boolean;
  onProfileRefresh: () => Promise<IUser | undefined>;
...
const ProfileAvatarBanner = ({
  ...
  const handleAvatarSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(tCommon('toast.successTitle'), {
        description: t('toast.avatarSuccess'),
      });
    } catch {
      onFileErrorChange(t('image.errors.invalidType'));
      toast.danger(tCommon('toast.errorTitle'), {
  ...
  const isActionDisabled = isUploadingAvatar || isPending;
    ...
    <div className="border-border bg-surface relative overflow-hidden rounded-xl border p-4 shadow-sm lg:p-5">
      ...
      <div className="flex min-w-0 items-center gap-3 lg:gap-4">
        <div className="relative shrink-0">
          <div className="border-border bg-primary-50 relative flex size-20 items-center justify-center overflow-hidden rounded-xl border lg:size-24">
            {user.profileImageUrl ? (
              ...
              <span className="text-h3 text-accent font-semibold">
                {initials}
              </span>
            )}
